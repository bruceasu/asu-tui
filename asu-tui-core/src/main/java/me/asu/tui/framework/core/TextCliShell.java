...
import static me.asu.tui.framework.api.CliContext.*;
...
import me.asu.tui.framework.api.CliPrompt;
import me.asu.tui.framework.core.api.CliShell;

/**
 * @author suk
 */
public final class TextCliShell implements CliShell
{
    ...
    public void exec(CliContext plug) {
        ...
        String[] args = (String[]) ctx.getValue(KEY_COMMAND_LINE_ARGS);
    ...
    private void activePrompt()
    {
        ...
        prompt = (prompts.size() > 0) ? prompts.get(0) : new TextCliPrompt();
        try{
            prompt.plug(ctx);
        }catch(Exception ex){
                ...
                while (loopRunning.get()) {
                    ...
                    if (controllersExist()) {
                        for (CliController controller : controllers) {
                            ...
                            if(enabled){
                                try{
                                    boolean ctrlResult = controller.handle(TextCliShell.this.ctx);
